document.addEventListener('DOMContentLoaded', () => {
    ...
    pane.addInput(PARAMS,
        'split', { min: 0, max: 20, step: 0.5, label: '分割数' })
        .on('change', (ev) => {
            if (ev.last) {
                allImgRemove();
    ...
    async function downloadAllImagesAsZip() {
        if (app.length === 0) {
            errorMessage("ダウンロードする画像がありません");
            return;
        }

        alertMessage("ZIPを作成中...");
        ...
        try {
            ...
            const addToZip = app.map((instance, i) => {
                return new Promise((resolve, reject) => {
                    instance.renderer.render(instance.stage);
                    const base64 = instance.renderer.extract.base64();
                    const data = base64.replace(/^data:image\/png;base64,/, '');
                    const filename = `saidan_${i + 1}_${ts}.png`;
                    zip.file(filename, data, { base64: true });
                    resolve();
                });
            });

            await Promise.all(addToZip);

            const blob = await zip.generateAsync({ type: 'blob' });
            saveAs(blob, `saidan_images_${ts}.zip`);
            alertMessage("ダウンロードが完了しました！");
        } catch (error) {
            console.error(error);
            errorMessage("ZIPの作成に失敗しました: " + error.message);
        }
    }
